...
static const char *logpath 	= "/.local/share/dailies/log";
...
main(int argc, char *argv[])
{
	...
	int nel = sizeof(TARGETS) / sizeof(Target);
	if (nel < 1) {
		write_to_slot(mname, "no study targets");
		return 1;
	}
...
#ifdef ___OPENBSD___
	if (unveil(path, "rw") == -1) {
		write_to_slot(mname, "unveil");
		exit(1);
	}
	if (unveil(NULL, NULL) == -1) { // lock unveil
		write_to_slot(mname, "unveil lock");
		exit(1);
	}
	...
	if (!log) { 
		write_to_slot(mname, "fopen dailies log"); 
		exit(1); 
	}

	/* 		   "000" + '|'     date */
	int max_line_len = (3 + 1) * nel + 10 + 1;
	...
	if (!date_field) { 
		write_to_slot(mname, "parsing date field"); exit(1); }
	date_field += 1;

	char *tok = strtok(date_field, "/");
	if (!tok) { write_to_slot(mname, "strtok date year"); exit(1); }
	int yy = atoi(tok);
	tok = strtok(NULL, "/");
	if (!tok) { write_to_slot(mname, "strtok date month"); exit(1); }
	int mm = atoi(tok);
	tok = strtok(NULL, "|");
	if (!tok) { write_to_slot(mname, "strtok date day"); exit(1); }
	int dd = atoi(tok);
	...
	for (int i = 0; i < nel; ++i) {
		...
		if (!tok) { 
			write_to_slot(mname, "dailies parsing"); 
			exit(1); 
		}
		dailies[i] = atoi(tok);
	}
...
skip_parse:
	...
	for (int i = 1; i < argc; i += 2) {
		...
		if (!found) {
			write_to_slot(mname, "unknown domain flag: -%c\n", flag);
			return 1;
		}
	}
...
skip_update:
	...
	for (int i = 0; i < nel; ++i) {
		...
		total_len += lenbuf;
		strncat(update_msg, buf, lenbuf);
	}

	if (total_len > 1)
		update_msg[total_len-2] = '\0'; /* remove last space */

	write_to_slot(mname, "%s", update_msg);
